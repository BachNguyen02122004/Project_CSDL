...
$index = $_GET['index'];
$arr = ['Laptop', 'Điện thoại', 'Tai nghe', 'Chuột', 'Bàn phím cơ', 'Đồng hồ', 'Tivi', 'Máy ảnh'];
$query = "SELECT product.id AS ID, product_image.image, product.name, product.price
FROM ((product
INNER JOIN product_image ON product.id = product_image.id)
INNER JOIN productline ON product.productline_id = productline.id)
WHERE productline.productline_name = '$arr[$index]' and product_image.is_default = 1;
";
